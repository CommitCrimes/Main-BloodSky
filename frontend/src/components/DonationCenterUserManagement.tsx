...
import { donationCenterAdminApi, type DonationCenterUser, type InviteDonationCenterUserRequest, type UpdateDonationCenterUserRequest } from '@/api/donationCenterAdmin';

interface DonationCenterUserManagementProps {
...
const DonationCenterUserManagement: React.FC<DonationCenterUserManagementProps> = ({ donationCenterId }) => {
  const [users, setUsers] = useState<DonationCenterUser[]>([]);
  const [loading, setLoading] = useState(true);
  const [openInviteDialog, setOpenInviteDialog] = useState(false);
  const [openEditDialog, setOpenEditDialog] = useState(false);
  const [selectedUser, setSelectedUser] = useState<DonationCenterUser | null>(null);
  ...
  const loadUsers = useCallback(async () => {
    try {
      setLoading(true);
      const response = await donationCenterAdminApi.getUsers(donationCenterId);
  ...
  const handleInviteSubmit = async () => {
    try {
      const userData: InviteDonationCenterUserRequest = {
      ...
      await donationCenterAdminApi.inviteUser(donationCenterId, userData);
  ...
  // Gestion du form d'edition
  const handleEditSubmit = async () => {
    if (!selectedUser) return;

    try {
      const userData: UpdateDonationCenterUserRequest = {
      ...
      await donationCenterAdminApi.updateUser(donationCenterId, selectedUser.userId, userData);
  ...
  const handleDelete = async (user: DonationCenterUser) => {
    if (!confirm(`Êtes-vous sûr de vouloir supprimer l'utilisateur ${user.userName} ${user.userFirstname} ?`)) {
      return;
    }

    try {
      await donationCenterAdminApi.deleteUser(donationCenterId, user.userId);
  ...
  const openEditDialogForUser = (user: DonationCenterUser) => {
...
export default DonationCenterUserManagement;